...
void vector_delete(vector_t *v){
    ...
    if(v){                /* if there is v*/
        free(v->data);    /* free the malloc from retval-> data*/
        free(v);          /*free the malloc from retval*/
    }
...
void vector_set(vector_t *v, size_t loc, int value) {
    ...
    if(v == NULL) { /* v when marked null*/
        fprintf(stderr, "vector_get: passed a NULL vector.\n");
        abort();
    }
    if (loc<v->size) /* v to loc*/
        v->data[loc]=value;
    else {
        ...
        int i; /*goes by each location to unset*/
        for (i=0;i<(loc+1);i++){
            if (i < v->size)
                temp[i]=v->data[i];
            else if (i!=loc)
        ...
        v->data=temp;
        v->size=loc+1;
        temp=NULL;/* remove temp by setting to null*/
    }
